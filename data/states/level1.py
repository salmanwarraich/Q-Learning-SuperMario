...
class Level1(tools._State):
    def __init__(self):
        ...
        self.mile_stone_3 = 0
        self.mile_stone_4 = 0
        self.last_mario_pos_X = 0
        self.last_mario_pos_Y = 0
        self.stuck_ctr = 0
        self.goal = False
    ...
    def setup_ground(self):
        ...
        sprites to walk on"""
        ground_rect1 = collider.Collider(0, c.GROUND_HEIGHT,    2953, 60) # 2993
        ground_rect2 = collider.Collider(3048, c.GROUND_HEIGHT,  690, 60)  #635
        ground_rect3 = collider.Collider(3819, c.GROUND_HEIGHT, 2735, 60) # 3753 , 2801
    ...
    def get_discretize_state(self):

        #discrete_state = discrete_states(0,0)
        reward_d = 0
        score = self.get_score()
        #pos_x = self.last_x_position
        #is_stuck = False
        #if self.mario.x_vel == 0 and self.mario.y_vel == 0:
        #    is_stuck = True
        level_completed = False
        if self.mario.rect.x >= 8480 and self.goal == False:
            self.goal = True
            reward_d += 8480
        ...
        if self.mario.rect.x >= 3048:
            #print(self.mario.rect.x)
            Stage_completed = True

        if self.mario.rect.x > self.max_pos_x:
            reward_d += (self.mario.rect.x - self.max_pos_x)*2 #0.01
            #pdb.set_trace()
            self.max_pos_x = self.mario.rect.x
            self.stuck_ctr = self.stuck_ctr + 1
        else:
            self.stuck_ctr = 0

        change_x = self.mario.rect.x - self.last_mario_pos_X
        change_y = self.last_mario_pos_Y - self.mario.rect.y

        if change_x < 0:
            reward_d = reward_d - 0.05
        self.last_mario_pos_X = self.mario.rect.x
        self.last_mario_pos_Y = self.mario.rect.y



        is_small_big = self.mario.big
        is_fire = self.mario.fire
        ...
        #print([self.mario.x_vel, self.mario.y_vel])
        pos_Const = 1
        neg_Const = -1
        if self.mario.x_vel > pos_Const:
            moving_right = True
        if self.mario.x_vel < neg_Const:
            moving_left = True
        if self.mario.y_vel > pos_Const:
            moving_up = True
        if self.mario.y_vel < neg_Const:
            moving_down = True

        if (self.mario.x_vel > pos_Const) and (self.mario.y_vel > pos_Const):
            moving_right_up = True
        if (self.mario.x_vel > pos_Const) and (self.mario.y_vel < neg_Const):
            moving_right_down = True
        if (self.mario.x_vel < neg_Const) and (self.mario.y_vel > pos_Const):
            moving_left_up = True
        if (self.mario.x_vel < neg_Const) and (self.mario.y_vel < neg_Const):
            moving_left_down = True

        if (abs(self.mario.x_vel) <= pos_Const) and (abs(self.mario.y_vel) <= neg_Const):
            not_moving = True

        if self.mario.x_vel >= 4.0:
            high_speed = True

        is_stuck = False
        if self.stuck_ctr >= 200:
            is_stuck = True
            self.stuck_ctr = 200
            if self.goal == False:
                reward_d = reward_d - 0.05
        ...
        on_ground = True
        if (self.mario.rect.y < (c.GROUND_HEIGHT - self.mario.rect.height)) and (self.mario.y_vel == 0):
            on_ground = False
            reward_d = reward_d + 0.01
        ...
        collided = False
        for enemy in self.enemy_group:
            distance1 = abs(enemy.rect.x - self.mario.rect.x)
            #distance2 = abs(self.mario.rect.x - enemy.rect.x - enemy.rect.width)
            #print([distance1])
        ...
        near_enemy = False
        midrange_enemy = False
        far_enemy = False

        for enemy in self.enemy_group:
            distance_x = abs(enemy.rect.x - self.mario.rect.x)
            distance_y = abs(enemy.rect.y - self.mario.rect.y)
            #print([distance_x, distance_y])
            if distance_x <= (self.mario.rect.width * 3) and distance_y <= (self.mario.rect.height * 3):
                near_enemy = True
                #print('near enemy')
            if distance_x <= (self.mario.rect.width * 6) and distance_y <= (self.mario.rect.height * 6):
                midrange_enemy = True
                #print('mid enemy')
            if distance_x <= (self.mario.rect.width * 9) and distance_y <= (self.mario.rect.height * 9):
                far_enemy = True
                #print('far enemy')
            #pdb.set_trace()
        ...
        for pipe in self.pipe_group:
            pipe_distance = pipe.rect.x - self.mario.rect.x - self.mario.rect.width
            if pipe_distance < 15 and pipe_distance >= 0:
                if pipe.rect.h > 160:
                    large_hurdle = True
                    #print('large hurdle')
                elif pipe.rect.h > 130:
                    mid_hurdle = True
                    #print('mid hurdle')
                elif pipe.rect.h > 70:
                    small_hurdle = True
        ...
        for step in self.step_group:
            step_distance = step.rect.x - self.mario.rect.x - self.mario.rect.width
            if step_distance < 20 and step_distance >= 0:
                small_hurdle = True
                mid_hurdle = True
                large_hurdle = True
                #print('step_distance')
        ...
        for pole in self.flag_pole_group:
            pole_distance = pole.rect.x - self.mario.rect.x - self.mario.rect.width
            if pole_distance < 40 and pole_distance >= 0:
                mid_hurdle = True
                large_hurdle = True
                print('pole')
                #large_hurdle = True
        ...
        hole_front = False
        for hole in self.ground_group:
            hole_distance =  hole.rect.x + hole.rect.w - self.mario.rect.x - self.mario.rect.width
            if hole_distance < 30 and hole_distance >= 0:
                hole_front = True
                #pdb.set_trace()

        hole_back = False
        for hole in self.ground_group:
            hole_distance =  hole.rect.x + hole.rect.w - self.mario.rect.x
            #print(hole_distance)
            if hole_distance > -135 and hole_distance <= -80:
                hole_back = True
                #print('hole_back True')
            #else:
            #    print('hole_back False')
                #pdb.set_trace()

        brick_coin_mid_near = False
        brick_coin_high_near = False
        for brick in self.brick_group:
            brick_distance =  brick.rect.x - self.mario.rect.x
            if brick_distance <= 50 and brick_distance >= -50:
                if brick.rect.y < 250:
                    brick_coin_mid_near = True
                    #print('mid brick')
                else:
                    brick_coin_high_near = True
                    #print('top brick')
            #else:
            #    print('no brick')
                #pdb.set_trace()

        for coin in self.coin_box_group:
            coin_distance =  coin.rect.x - self.mario.rect.x
            if coin_distance <= 50 and coin_distance >= -50:
                if coin.rect.y < 250:
                    brick_coin_mid_near = True
                    #print('mid coin')
                else:
        ...
        #pdb.set_trace()
        # binary mapping
        bin_state = 0b00000000000000000000000000000000

        if is_small_big:
            bin_state = bin_state | 0b00000000000000000000000000000001

        if is_fire:
            bin_state = bin_state | 0b00000000000000000000000000000010

        if moving_right:
            bin_state = bin_state | 0b00000000000000000000000000000100

        if moving_right_up:
            bin_state = bin_state | 0b00000000000000000000000000001000

        if moving_right_down:
            bin_state = bin_state | 0b00000000000000000000000000010000

        if moving_left:
            bin_state = bin_state | 0b00000000000000000000000000100000

        if moving_left_up:
            bin_state = bin_state | 0b00000000000000000000000001000000

        if moving_left_down:
            bin_state = bin_state | 0b00000000000000000000000010000000

        if moving_up:
            bin_state = bin_state | 0b00000000000000000000000100000000

        if moving_down:
            bin_state = bin_state | 0b00000000000000000000001000000000

        if not_moving:
            bin_state = bin_state | 0b00000000000000000000010000000000

        if is_stuck:
            bin_state = bin_state | 0b00000000000000000000100000000000

        if on_ground:
            bin_state = bin_state | 0b00000000000000000001000000000000

        if collided:
            bin_state = bin_state | 0b00000000000000000010000000000000

        if near_enemy:
            bin_state = bin_state | 0b00000000000000000100000000000000

        if midrange_enemy:
            bin_state = bin_state | 0b00000000000000001000000000000000

        if far_enemy:
            bin_state = bin_state | 0b00000000000000010000000000000000

        if small_hurdle:
            bin_state = bin_state | 0b00000000000000100000000000000000

        if mid_hurdle:
            bin_state = bin_state | 0b00000000000001000000000000000000

        if large_hurdle:
            bin_state = bin_state | 0b00000000000010000000000000000000

        if hole_front:
            bin_state = bin_state | 0b00000000000100000000000000000000

        if hole_back:
            bin_state = bin_state | 0b00000000001000000000000000000000


        if brick_coin_mid_near:
            bin_state = bin_state | 0b00000000010000000000000000000000
            #print('brick mid')

        if brick_coin_high_near:
            bin_state = bin_state | 0b00000000100000000000000000000000
            #print('brick top')

        #if high_speed:
        #    bin_state = bin_state | 0b00000001000000000000000000000000

        #if Stage_completed:
        #    bin_state = bin_state | 0b00000001000000000000000000000000
        if level_completed:
            bin_state = bin_state | 0b11111111111111111111111111111111
        ...
        '''
                ...
                hole_front,
                brick_coin_mid_near,
                brick_coin_high_near]
        '''
        #pdb.set_trace()
        return (reward_d, bin_state)
